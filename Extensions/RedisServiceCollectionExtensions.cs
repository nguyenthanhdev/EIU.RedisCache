...
using EIU.Caching.Redis.Manager;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Options;
using StackExchange.Redis;

namespace EIU.Caching.Redis.Extensions
{
    /// <summary>
    ...
    public static class RedisServiceCollectionExtensions
    {
        ...
        )
        {
            ...
            {
                Console.WriteLine(
                    "⚠️ RedisCacheConfig.Enabled = false → Bộ nhớ đệm Redis sẽ không được kích hoạt."
                );
                return services;
            }
            ...
            if (string.IsNullOrWhiteSpace(redisConfig.ConnectionString))
            {
                throw new InvalidOperationException(
                    "❌ Cấu hình RedisCacheConfig.ConnectionString bị thiếu hoặc rỗng. Vui lòng kiểm tra lại file appsettings.json!"
                );
            }
            ...
            else
            {
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine(
                    "⚠️ Redis không khả dụng → RedisCacheService sẽ không được đăng ký."
                );
                Console.ResetColor();
            }
            return services;
